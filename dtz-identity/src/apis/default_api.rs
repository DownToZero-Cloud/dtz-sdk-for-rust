...
/// struct for typed errors of method [`token_request`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn oauth_authorize(configuration: &Configuration, response_type: &str, client_id: &str, redirect_uri: &str, scope: &str, state: Option<&str>, nonce: Option<&str>) -> Result<(), Error<OauthAuthorizeError>> {
    let local_var_configuration = configuration;
...
/// token request
pub async fn token_request(configuration: &Configuration, oauth_token_request: crate::models::OauthTokenRequest) -> Result<models::TokenResponse, Error<TokenRequestError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<TokenRequestError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: Some(crate::apis::Content::Text(local_var_content)), entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
